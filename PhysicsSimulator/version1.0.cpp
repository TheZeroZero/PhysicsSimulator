...
const double GRAV = .0001; //gravitational constant
const double RESTITUTION = .5;
const int CIRCLES_COUNT = 20;
...
PhysicsCircle::PhysicsCircle(double x, double y, int radius, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	...
	m_y = y;
	m_radius = radius;
	m_mass = std::_Pi * m_radius * m_radius;
	m_dx = 0;
	m_dy = 0;
	m_r = r;
	m_g = g;
...
void PhysicsCircle::update(std::vector<PhysicsCircle>& circles)
{
	...
	for (int i{ 0 }; i < circles.size(); ++i)
	{
		...
		else
		{
			...
			{
				m_dx = -RESTITUTION * m_dx;
				m_dy = -RESTITUTION * m_dy;

				//circles[i].m_dx = -RESTITUTION * circles[i].m_dx;
				//circles[i].m_dy = -RESTITUTION * circles[i].m_dy;
			}*/
	...
	else
	{
		...
		else
		{
			...
			for (int i{ 0 }; i < CIRCLES_COUNT; ++i)
			{
				...
				circles.push_back(PhysicsCircle(x ,y, radius, r, g, b, a));
			}
			while (!quit)
			{
