...
void PhysicsBall::update(std::vector<PhysicsBall>& otherBalls)
{
	...
	for(PhysicsBall& otherBall : otherBalls)
	{
		if (&otherBall != this && checkCollision(otherBall))
		{
			...
			Vector_2d newVelocity;
			newVelocity.x = projectVector(m_velocity, flippingAxis);
			newVelocity.y = -projectVector(m_velocity, perpendicularFlippingAxis);

			Vector_2d perpendicularVelocity;
			Vector_2d parallelVelocity;
			perpendicularVelocity = normalizeVector(perpendicularFlippingAxis) * newVelocity.y;
			parallelVelocity = normalizeVector(flippingAxis) * newVelocity.x;

			newVelocity = perpendicularVelocity + parallelVelocity;

			//m_velocity = RESTITUTION * ((m_mass - otherBall.m_mass) * newVelocity + 2 * otherBall.m_mass * otherBall.m_velocity) / (m_mass + otherBall.m_mass);
			m_velocity = RESTITUTION * newVelocity;
			...
			//aproximate the position of our ball, when the collision occurs
			//collisionPosition = otherBall.m_position - otherBall.m_velocity / 2;
			//perpendicularFlippingAxis = getVectorFromPositions(m_position, collisionPosition);
			//flippingAxis = getPerpendicularVector(perpendicularFlippingAxis);

			//put the ball out of collision
			//otherBall.m_position -= m_velocity;

			//flip the velocity vector
			newVelocity.x = projectVector(otherBall.m_velocity, flippingAxis);
			newVelocity.y = -projectVector(otherBall.m_velocity, perpendicularFlippingAxis);
			...
			otherBall.m_velocity = RESTITUTION * newVelocity;
		}
	}
